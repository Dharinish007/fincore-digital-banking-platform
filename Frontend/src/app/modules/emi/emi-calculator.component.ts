...
export class EmiCalculatorComponent implements OnInit {
  ...
  calculate(): void {
    ...
    // Trigger backend EMI calculation engine & database audit trail
    const tenureMonths = formVal.tenureType === 'YEARS' ? Number(formVal.tenure) * 12 : Number(formVal.tenure);
    this.api.post('/api/operations/emi', {
      principal: Number(formVal.loanAmount),
      annualRate: Number(formVal.interestRate),
      tenureMonths: tenureMonths
    }).subscribe({
      error: (err) => console.warn('Backend EMI audit sync:', err)
    });
  }
