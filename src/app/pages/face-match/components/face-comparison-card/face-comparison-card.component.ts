...
export class FaceComparisonCardComponent implements OnInit, OnDestroy {
  ...
  @Output() compareFaces = new EventEmitter<{
    idFile: File;
    selfieFile: File;
  }>();
  ...
  @ViewChild("snapshotCanvas")
  snapshotCanvasRef!: ElementRef<HTMLCanvasElement>;
  ...
  private readonly maxSizeBytes =
    environment.maxUploadSizeBytes || 10 * 1024 * 1024; // 10MB
  ...
  public useSnapshot(): void {
    ...
    const file = new File([this.capturedBlob], `selfie_${timestamp}.jpg`, {
      type: "image/jpeg",
    });
    this.closeCamera();
  ...
  public closeCameraOnBackdrop(event: MouseEvent): void {
    ...
    if (
      (event.target as HTMLElement).classList.contains("camera-modal-overlay")
    ) {
      this.closeCamera();
    }
  }
  ...
  private handleFile(file: File, type: "id" | "selfie"): void {
    ...
    const isTypeValid =
      this.allowedTypes.includes(fileType) || isExtensionValid;
  ...
  public removeIdImage(event?: MouseEvent): void {
    ...
    if (
      this.idState.previewUrl &&
      this.idState.previewUrl.startsWith("blob:")
    ) {
  ...
  public removeSelfieImage(event?: MouseEvent): void {
    ...
    if (
      this.selfieState.previewUrl &&
      this.selfieState.previewUrl.startsWith("blob:")
    ) {
