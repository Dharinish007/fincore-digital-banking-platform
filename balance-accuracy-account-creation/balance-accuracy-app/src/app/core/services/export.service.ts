...
export class ExportService {
  ...
  public exportToCSV(data: BankAccount[] | Transaction[] | any[], filename = 'FinCore_Report.csv'): void {
    ...
    if ('accountNumber' in first && 'ledgerBalance' in first) {
      // BankAccount
      headers = [
        'Account Number', 'Customer Name', 'Customer ID', 'Branch',
        'Account Type', 'Ledger Balance ($)', 'Available Balance ($)',
        'System Calculated ($)', 'Difference ($)', 'Status', 'Last Verified'
      ];
      rows = (data as BankAccount[]).map(acc => [
        `"${acc.accountNumber}"`, `"${acc.customerName}"`, `"${acc.customerId}"`,
        `"${acc.branch}"`, `"${acc.accountType}"`, acc.ledgerBalance.toFixed(2),
        acc.availableBalance.toFixed(2), acc.systemCalculatedBalance.toFixed(2),
        acc.difference.toFixed(2), `"${acc.status}"`, `"${acc.lastVerified}"`
      ]);
    } else {
      // Transaction or generic
      headers = [
        'Transaction ID', 'Sender Account', 'Receiver Account',
        'Type', 'Amount ($)', 'Date', 'Reference', 'Status', 'Charges ($)', 'Description'
      ];
      rows = (data as Transaction[]).map(tx => [
        `"${tx.id}"`, `"${tx.sender}"`, `"${tx.receiver}"`,
        `"${tx.type}"`, (tx.amount || 0).toFixed(2), `"${tx.date}"`,
        `"${tx.reference}"`, `"${tx.status}"`, (tx.charges || 0).toFixed(2),
        `"${tx.description || ''}"`
      ]);
    }
  ...
  public exportToPDF(data: BankAccount[] | Transaction[] | any[], filename = 'FinCore_Audit_Report.pdf'): void {
    if (!data || data.length === 0) return;
