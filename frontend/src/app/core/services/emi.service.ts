...
interface BackendEMICalculationResponse {
  monthlyEmi: number;
  totalInterest: number;
  totalPayable: number;
}
...
export class EmiService {
  ...
  ): Observable<EMICalculationResponse> {
    return this.http
      .post<BackendEMICalculationResponse>(this.apiUrl, request)
      .pipe(
        map((response) => ({
          monthlyEMI: Number(response.monthlyEmi),
          totalInterest: Number(response.totalInterest),
          totalPayment: Number(response.totalPayable),
        })),
        catchError(() => {
