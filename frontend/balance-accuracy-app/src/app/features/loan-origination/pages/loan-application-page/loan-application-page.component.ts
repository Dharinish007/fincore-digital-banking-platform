...
export class LoanApplicationPageComponent implements OnInit {
  ...
  checkPreQualificationHandover() {
    ...
    const preQual: PreQualificationData | null =
      nav?.preQualified || this.mockData.getPendingPreQualification();
  ...
  get mandatoryDocsUploadedCount(): number {
    return this.documents.filter(
      (d) =>
        d.mandatory && (d.status === "Uploaded" || d.status === "Verified"),
    ).length;
  }
  ...
  get uploadProgressPercent(): number {
    ...
    return Math.round(
      (this.mandatoryDocsUploadedCount / this.mandatoryDocsCount) * 100,
    );
  }
  ...
  processFileForDoc(file: File, doc: UploadedDoc) {
    ...
    doc.uploadedAt = new Date().toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
  ...
  addCustomDocPrompt() {
    const docName = prompt(
      "Enter document title (e.g. Property Sale Agreement, Form 16, Tax Returns, Business Proof):",
    );
  ...
  saveAsDraft() {
    ...
    this.loanService.createLoanApplication(payload).subscribe({
      next: (createdLoan) => {
        this.applicationId = String(createdLoan.loanId || generatedId);
        this.draftSaved = true;
        this.statusMessage = `Application successfully saved as Draft! Reference: #LO-${this.applicationId}. You can resume or submit anytime from Applications.`;
      },
      error: () => {
        this.errorMessage =
          "Draft could not be saved. Please check that the backend and database are running and try again.";
      },
    });
  }
  ...
  openReviewModal() {
    ...
    if (this.form.invalid) {
      this.errorMessage =
        "Please complete all required fields across the form sections before reviewing.";
      return;
    }
    this.errorMessage = "";
    ...
    this.loanService.createLoanApplication(payload).subscribe({
      next: (createdLoan) => {
        ...
        this.applicationId = String(
          createdLoan.loanId || Math.floor(1005 + Math.random() * 8990),
        );
      ...
      error: () => {
        ...
        this.errorMessage =
          "Application could not be saved. Please check that the backend and database are running and try again.";
      },
    });
  }
