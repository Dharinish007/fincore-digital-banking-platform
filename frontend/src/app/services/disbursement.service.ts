...
export class DisbursementService {
  ...
  loadFromBackend(): void {
    // Fetch live loans from PostgreSQL database
    this.api.get<any[]>('/api/operations/loans').subscribe({
      next: (dbLoans) => {
        if (dbLoans && dbLoans.length > 0) {
          const mappedLoans: LoanDetails[] = dbLoans.map(l => ({
            loanId: l.loanId || `LN1000${l.id}`,
            customerId: `CUST10${l.customerId}`,
            customerName: l.customerName || 'Borrower',
            loanType: l.loanType || 'Home Loan',
            sanctionedAmount: Number(l.sanctionedAmount || 500000),
            disbursedAmount: Number(l.disbursedAmount || 400000),
            remainingAmount: Number(l.remainingAmount || 100000),
            outstandingPrincipal: Number(l.outstandingPrincipal || 400000),
            paidAmount: 12000,
            creditScore: 780,
            status: l.status || 'ACTIVE',
            emiStatus: 'DUE',
            nextEmiDate: '05-Oct-2026',
            interestRate: Number(l.interestRate || 8.5),
            tenureMonths: Number(l.tenureMonths || 60),
            monthlyEmi: Number(l.monthlyEmi || 8678),
            startDate: '2026-01-15',
            autoDebitEnabled: true,
            kafkaSagaStatus: 'Account -> Loan -> Payment'
          }));
          this.loans = mappedLoans;
          this.loans$.next([...this.loans]);
        }
      },
      error: (err) => console.warn('Loans DB load fallback:', err)
    });

    // Fetch live disbursements from PostgreSQL database
    this.api.get<any[]>('/api/operations/disbursements').subscribe({
      next: (records) => {
        if (records && records.length > 0) {
          const dbDisbursements: DisbursementRecord[] = records.map(r => ({
            disbursementId: `DISB-DB-${r.id}`,
            loanId: `LN1000${r.loanId}`,
            customerName: 'Verified Borrower',
            disbursementAmount: Number(r.amount),
            beneficiaryAccount: 'ACC-8849-1001',
            disbursementDate: r.disbursedAt ? String(r.disbursedAt).substring(0, 10) : '2026-09-12',
            disbursementMode: (r.channel as any) || 'RTGS',
            status: 'SUCCESS',
            referenceNumber: r.reference || `REF-${r.id}`,
            remarks: 'Disbursed via Core Settlement Engine'
          }));
          this.disbursements = dbDisbursements;
          this.disbursements$.next([...this.disbursements]);
        }
      },
      error: (err) => console.warn('Using local disbursements cache:', err)
    });
  }
  ...
  processDisbursement(req: DisbursementRequest): { success: boolean; message: string; record?: DisbursementRecord } {
    ...
    const numLoanId = parseInt(loan.loanId.replace(/\D/g, ''), 10) || 1;
    this.api.post('/api/operations/disbursements', {
      loanId: numLoanId,
      amount: req.disbursementAmount,
      channel: req.disbursementMode || 'RTGS'
    }).subscribe({
      next: (res) => console.log('Loan disbursement synced to PostgreSQL DB:', res),
      error: (err) => console.warn('Disbursement DB sync fallback:', err)
    });
