...
public class AuditLogController {
    ...
    public ResponseEntity<AuditLogPage> getAll(
        ...
        List<AuditLog> filtered = repository.findAllByOrderByCreatedAtDesc().stream()
                .filter(log -> normalizedModule.isBlank() || 
                        log.getModule().toLowerCase(Locale.ROOT).contains(normalizedModule) ||
                        (log.getEntityType() != null && log.getEntityType().toLowerCase(Locale.ROOT).contains(normalizedModule)))
