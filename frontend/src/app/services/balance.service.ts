...
export class BalanceService {
  ...
  deposit(amount: number, description: string = 'Cash / Check Deposit') {
    ...
    // Sync balance adjustment to PostgreSQL database
    const numericId = parseInt(acc.id.replace(/\D/g, ''), 10) || 1;
    this.api.post(`/api/operations/accounts/${numericId}/balance-adjustments`, {
      amount,
      entryType: 'CREDIT',
      description
    }).subscribe({
      error: (err) => console.warn('Could not sync deposit to backend DB:', err)
    });
  ...
  withdraw(amount: number, description: string = 'Withdrawal / Debit') {
    ...
    this.api.post(`/api/operations/accounts/${numericId}/balance-adjustments`, {
      ...
      error: (err) => console.warn('Could not sync withdrawal to backend DB:', err)
    });
