...
export class TransactionService {
  ...
  readonly backendTransactions = signal<Transaction[]>([]);
  ...
  loadStatements(accId?: string): void {
    const targetId = accId || this.accountService.activeAccountId();
    const numId = parseInt(targetId.replace(/\D/g, ''), 10) || 1;
    this.api.get<any[]>(`/api/operations/accounts/${numId}/statement`).subscribe({
      next: (stmts) => {
        if (stmts && stmts.length > 0) {
          const mapped: Transaction[] = stmts.map(s => ({
            id: `stmt-${s.id}`,
            date: s.createdAt ? String(s.createdAt).substring(0, 10) : new Date().toISOString().substring(0, 10),
            referenceId: s.reference || `STMT-1000${s.id}`,
            description: s.description || 'Certified Account Statement Transaction',
            category: s.entryType === 'CREDIT' ? 'Income' : 'Operations',
            type: (s.entryType === 'CREDIT' ? 'CREDIT' : 'DEBIT') as TransactionType,
            amount: Number(s.amount),
            balanceAfter: Number(s.balanceAfter || 0),
            status: 'COMPLETED'
          }));
          this.backendTransactions.set(mapped);
        }
      },
      error: (err) => console.warn('Using local statement cache:', err)
    });
  }

  readonly currentAccountTransactions = computed(() => {
    const accId = this.accountService.activeAccountId();
    const dbTxs = this.backendTransactions();
