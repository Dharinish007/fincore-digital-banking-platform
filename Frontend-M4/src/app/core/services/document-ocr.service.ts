...
export class DocumentOcrService {
    ...
    if (file.type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf')) {
      return of<OcrProcessingResult>({
        ...
        error: {
          ...
          message: 'Client-side Wasm OCR requires image formats (JPG, JPEG, PNG). Please convert or save your document as an image file before uploading.',
  ...
  private async performRealClientSideOcr(file: File, documentType: SupportedDocumentType): Promise<OcrProcessingResult> {
    ...
    try {
      ...
      try {
        ...
        console.warn('[DocumentOcrService] Default worker init failed, attempting fallback CDN config:', workerErr);
  ...
  private parseDocumentFields(rawText: string, documentType: SupportedDocumentType, confidence: number): OcrExtractedData {
    ...
    if (documentType === 'pan') {
      ...
      const cleanUpperNoSpaces = textUpper.replace(/([A-Z0-9])\s+([A-Z0-9])/g, '$1$2');
      const panRegex = /[A-Z]{5}[0-9]{4}[A-Z]{1}/;
      const panMatch = textUpper.match(panRegex) || cleanUpperNoSpaces.match(panRegex);
      ...
      const ignoreWords = ['INCOME', 'TAX', 'GOVT', 'GOVERNMENT', 'INDIA', 'DEPARTMENT', 'PERMANENT', 'ACCOUNT', 'CARD', 'SIGNATURE', 'FATHER', 'NAME', 'NUMBER', 'DOB', 'DATE'];
