...
export class CollectionService {
  ...
  loadFromBackend(): void {
    this.api.get<any[]>('/api/operations/collections').subscribe({
      next: (records) => {
        if (records && records.length > 0) {
          const dbCollections: PaymentRecord[] = records.map(r => ({
            paymentId: `PAY-DB-${r.id}`,
            loanId: `LN1000${r.loanId}`,
            customerName: 'Verified Borrower',
            paymentAmount: Number(r.amount),
            paymentDate: r.collectedAt ? String(r.collectedAt).substring(0, 10) : '2026-09-12',
            paymentMode: (r.channel as any) || 'UPI',
            referenceNumber: r.reference || `COL-DEMO-100${r.id}`,
            status: 'PAID',
            remarks: 'Repayment recorded via Backend Ledger'
          }));
          this.paymentHistory = dbCollections;
          this.history$.next([...this.paymentHistory]);
        }
      },
      error: (err) => console.warn('Backend collections check:', err)
    });
  }
  ...
  recordPayment(req: PaymentRequest): { success: boolean; message: string; record?: PaymentRecord } {
    ...
    // Persist repayment collection to PostgreSQL Database via Backend API
    const numLoanId = parseInt(req.loanId.replace(/\D/g, ''), 10) || 1;
    this.api.post('/api/operations/collections', {
      loanId: numLoanId,
      scheduleId: 1,
      amount: req.paymentAmount,
      channel: req.paymentMode || 'UPI'
    }).subscribe({
      next: (res) => console.log('Loan collection synced to PostgreSQL DB:', res),
      error: (err) => console.warn('Collection DB sync fallback:', err)
    });
