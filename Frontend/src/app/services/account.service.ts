...
export class AccountService {
  ...
  loadFromBackend(): void {
    this.api.get<any[]>('/api/operations/accounts').subscribe({
      next: (backendAccounts) => {
        if (backendAccounts && backendAccounts.length > 0) {
          const current = this.accounts();
          const mapped: Account[] = backendAccounts.map(ba => {
            const existing = current.find(c => c.accountNumber === ba.accountNumber || c.id === `acc-${ba.id}`);
            const custName = ba.customerName || 'John Smith';
            return {
              id: `acc-${ba.id}`,
              accountNumber: ba.accountNumber || `ACC-8849-100${ba.id}`,
              type: (ba.accountType as any) || 'SAVINGS',
              name: `${custName}'s ${ba.accountType || 'Savings'} Account`,
              balance: Number(ba.balance || 0),
              availableBalance: Number(ba.balance || 0),
              currency: 'INR',
              holderName: custName,
              holderSSN: '987-65-4321',
              email: ba.customerEmail || 'client@fincore.com',
              phone: ba.customerPhone || '+91 98765 43210',
              address: 'FinCore Financial District, Mumbai',
              ownershipStatus: 'VERIFIED_OWNER',
              status: (ba.status as any) || 'ACTIVE',
              kycStatus: 'VERIFIED',
              riskLevel: 'LOW',
              bankBranch: 'Mumbai Main Hub',
              routingNumber: '021000089',
              openedDate: ba.openedAt || new Date().toISOString().split('T')[0],
              minBalanceThreshold: 500,
              overdraftLimit: 5000,
              isOverdraftEligible: true,
          ...
          // Directly populate from database accounts
          this.accounts.set(mapped);
          if (mapped.length > 0 && !mapped.some(m => m.id === this.activeAccountId())) {
            this.activeAccountId.set(mapped[0].id);
          }
          this.saveAccountsToStorage(mapped);
        }
      },
      error: (err) => console.warn('Operating with local storage cache (Backend status check):', err)
    });
  }
  ...
  updateAccountStatus(id: string, newStatus: AccountStatus) {
    ...
    if (numericId) {
      this.api.post(`/api/operations/accounts/${numericId}/lifecycle`, { status: newStatus }).subscribe({
        next: () => {
          this.loadFromBackend();
        },
        error: (err) => console.warn('Could not sync lifecycle update to backend DB:', err)
      });
    }
  }
