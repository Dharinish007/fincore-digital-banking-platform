...
@Component({
  ...
  template: `
    <div class="filter-bar" [formGroup]="filterForm">
      <mat-form-field appearance="outline" class="filter-search">
        ...
        <input matInput formControlName="search" placeholder="Account number, customer name or ID">
      ...
      <mat-form-field appearance="outline" class="filter-select">
        ...
        <mat-select formControlName="branch">
          ...
          @for (b of branches; track b) { <mat-option [value]="b">{{ b }}</mat-option> }
        </mat-select>
      </mat-form-field>
  ...
  `,
  styles: [`
    .filter-bar { display: flex; flex-wrap: wrap; align-items: center; gap: 12px; padding: 16px 20px; background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: var(--radius-md); margin-bottom: 20px; }
...
export class AccountFilterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.filterForm = this.fb.group({ search: [''], status: [null], accountType: [null], branch: [null] });
  ...
  reset(): void { this.filterForm.reset({ search: '', status: null, accountType: null, branch: null }); }
