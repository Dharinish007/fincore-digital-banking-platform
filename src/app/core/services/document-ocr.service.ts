...
export class DocumentOcrService {
  ...
  validateDocumentFile(file: File): FileValidationResult {
    ...
    if (!isValidType) {
      return {
        valid: false,
        errorMessage: "Invalid file format. Supported formats: JPG, JPEG, PNG.",
      };
    }
  ...
  processDocument(
    ...
    if (!environment.mockFallback) {
      ...
      return this.http.post<BackendOcrResponse>(apiUrl, formData).pipe(
        map((response) => this.toProcessingResult(response)),
    ...
    return {
      ...
      data: response.success
        ? {
            ...
            documentType:
              response.documentType.toLowerCase() as SupportedDocumentType,
      ...
      error: response.success
        ...
        : {
            ...
            message:
              response.message ||
              "The OCR service could not process this document.",
          },
    };
  }
...
interface BackendOcrResponse {
  ...
  verificationStatus: OcrExtractedData["verificationStatus"];
  message?: string;
}
