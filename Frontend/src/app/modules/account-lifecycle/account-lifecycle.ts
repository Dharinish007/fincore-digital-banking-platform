    ...
    <div class="lifecycle-view">
      ...
      <div class="account-list-card">
        <div class="card-header-bar">
          <div>
            <h3>ALL SYSTEM ACCOUNTS (POSTGRESQL DATABASE: <span style="color: #38bdf8;">account</span>)</h3>
            <div class="card-header-sub">Live database accounts with direct lifecycle status transition controls</div>
          </div>
        ...
        <!-- Quick Status Lifecycle Control Bar for Selected Account -->
        <div style="display: flex; gap: 10px; align-items: center; margin-bottom: 1.25rem; background: #0f172a; padding: 12px 16px; border-radius: 8px; border: 1px solid #1e293b; flex-wrap: wrap;">
          <span style="font-size: 0.82rem; color: #94a3b8; font-weight: 700;">
            QUICK STATUS TRANSITION ({{ accountService.activeAccount().name }}):
          </span>
          <button type="button" class="btn-sm" style="background: #065f46; color: #a7f3d0; border: 1px solid #059669; cursor: pointer; padding: 5px 12px; border-radius: 4px; font-weight: 600;" (click)="changeStatus(accountService.activeAccountId(), 'ACTIVE')">
            🟢 Set ACTIVE
          </button>
          <button type="button" class="btn-sm" style="background: #7f1d1d; color: #fecaca; border: 1px solid #b91c1c; cursor: pointer; padding: 5px 12px; border-radius: 4px; font-weight: 600;" (click)="changeStatus(accountService.activeAccountId(), 'FROZEN')">
            ❄️ Freeze (Saga Lock)
          </button>
          <button type="button" class="btn-sm" style="background: #78350f; color: #fde68a; border: 1px solid #d97706; cursor: pointer; padding: 5px 12px; border-radius: 4px; font-weight: 600;" (click)="changeStatus(accountService.activeAccountId(), 'DORMANT')">
            💤 Set DORMANT
          </button>
          <button type="button" class="btn-sm" style="background: #1e3a8a; color: #bfdbfe; border: 1px solid #2563eb; cursor: pointer; padding: 5px 12px; border-radius: 4px; font-weight: 600;" (click)="changeStatus(accountService.activeAccountId(), 'PENDING_VERIFICATION')">
            ⏳ Set PENDING
          </button>
          <button type="button" class="btn-sm" style="background: #374151; color: #f3f4f6; border: 1px solid #4b5563; cursor: pointer; padding: 5px 12px; border-radius: 4px; font-weight: 600;" (click)="changeStatus(accountService.activeAccountId(), 'CLOSED')">
            🔒 Close Account
          </button>
        </div>
...
export class AccountLifecycleComponent {
  ...
  triggerEdit(accountId: string) {
    ...
    this.modalService.openEditAccountModal(accountId);
  }
