...
if(!$customer_id||!$match_result||$confidence===null){http_response_code(400);echo json_encode(["success"=>false,"message"=>"customer_id, match_result and confidence are required"]);exit;}
$sql="INSERT INTO face_match (customer_id,match_result,confidence) VALUES (?,?,?)";
$stmt=$conn->prepare($sql);if(!$stmt){http_response_code(500);echo json_encode(["success"=>false,"message"=>"Failed to prepare face match query"]);exit;}
