...
import com.example.milestone3.loanmanagement.collection.LoanEntity;
import com.example.milestone3.loanmanagement.collection.LoanRepo;
...
public class EMIService {
    ...
    public EmiCalculationResponse calculate(EmiCalculationRequest EmiCalculationRequest) {
        ...
        response.setMonthlyEmi(emiAmount);
        response.setTotalAmount(totalAmount);
        response.setTotalPayable(totalAmount);
        response.setTotalInterest(totalInterest);
        response.setPrincipalAmount(principal);
        response.setInterestRate(annual);
        response.setTenureMonths(months);
        response.setLoanId(EmiCalculationRequest.getLoanId());
        return response;
    }
