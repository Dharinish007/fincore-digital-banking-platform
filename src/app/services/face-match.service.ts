...
export class FaceMatchService {
  ...
  public compareFaces(
    ...
    const endpointUrl =
      environment.faceMatchEndpoint || `${environment.apiUrl}/face-match`;

    return this.http.post<FaceMatchApiResponse>(endpointUrl, formData).pipe(
   ...
   */
  private normalizeResult(res: FaceMatchApiResponse): FaceMatchResult {
    const matched = typeof res.matched === "boolean" ? res.matched : false;
    const threshold = typeof res.threshold === "number" ? res.threshold : 0;
    const score =
      typeof res.score === "number"
        ...
        : typeof res.distance === "number" && threshold > 0
          ...
          : matched
            ? 100
            : 0;
    const hasBackendDecision =
      typeof res.matched === "boolean" && typeof res.distance === "number";
    const status: VerificationState =
      res.status ||
      (res.message && !hasBackendDecision
        ? "ERROR"
        : matched
          ? "VERIFIED"
          : "REJECTED");
    ...
    return {
      ...
      auditRef:
        res.auditRef ||
        `FIN-KYC-${Math.floor(10000 + Math.random() * 90000)}-FM`,
  ...
  private handleApiError(error: HttpErrorResponse): FaceMatchResult {
    let message =
      "Face Match Could Not Be Completed. Unable to process the images. Please try again.";
    if (error.status === 0) {
      message =
        "Unable to connect to Face Match backend server. Please check backend connectivity or try again.";
    } else if (error.status === 400) {
      message =
        error.error?.message ||
        "Invalid image format or unreadable facial features. Please upload clearer images.";
    } else if (error.status === 413) {
      message =
        "Uploaded image file size exceeds the allowed server limit. Please upload images under 10MB.";
    ...
    return [
      {
        ...
        summary:
          (result.idFaceDetected ?? true)
            ...
            : "No face found in ID",
        details:
          (result.idFaceQuality ?? 95) >= 90
            ? "Face visibility: Excellent"
            : "Face visibility: Moderate",
        icon: "badge",
      },
      {
        id: "selfie-detection",
        ...
        summary:
          (result.selfieFaceDetected ?? true)
            ? "Single face detected"
            : "Multiple / No faces found",
        details:
          (result.selfieFaceQuality ?? 92) >= 90
            ? "Face visibility: Excellent"
            : "Face visibility: Acceptable",
        icon: "face",
      },
      {
        id: "blur-sharpness",
        ...
        status:
          (result.blurScore ?? 91) >= 85
            ? "GOOD"
            : (result.blurScore ?? 91) >= 70
              ? "WARNING"
              : "FAIL",
        score: result.blurScore ?? 91,
        summary:
          (result.blurScore ?? 91) >= 85
            ? "Low blur detected across both images"
            : "Minor blur detected on selfie",
      ...
      },
      {
        ...
        status:
          (result.poseScore ?? 96) >= 85
            ? "GOOD"
            : (result.poseScore ?? 96) >= 70
              ? "WARNING"
              : "FAIL",
        score: result.poseScore ?? 96,
        summary:
          (result.poseScore ?? 96) >= 85
            ? "Faces are aligned within acceptable angle"
            : "Yaw/pitch angle deviation detected",
    ...
    switch (state) {
      ...
      case "REJECTED":
        this.resultSubject.next({
          ...
          errorMessage:
            "Facial similarity score (62.4%) is below the required 80.0% threshold.",
          timestamp: new Date().toISOString(),
        });
        break;
      ...
      case "REVIEW_REQUIRED":
        this.resultSubject.next({
          ...
          poseScore: 79,
          errorMessage:
            "Borderline similarity score (76.5%) detected. Supervisor manual inspection required.",
          timestamp: new Date().toISOString(),
        });
        break;
...
interface FaceMatchApiResponse extends Partial<FaceMatchResult> {
  distance?: number;
  model?: string;
  message?: string;
}
